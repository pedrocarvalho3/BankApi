...
public class CreateWithdrawUseCaseTests
{
    [Fact]
    public async Task ExecuteAsync_ShouldCreateWithdrawTransaction()
    {
        var accountId = Guid.NewGuid();
        var ownerId = Guid.NewGuid();

        var fakeAccount = new BankAccount(ownerId, EAccountType.Current);
        fakeAccount.Deposit(200);
        ...
        var useCase = new CreateWithdrawUseCase(unitOfWork);
        ...
        Assert.Equal(ETransactionType.Withdraw, result.TransactionType);
